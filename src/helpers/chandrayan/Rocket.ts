...
export class Rocket {
  mesh: any;
  roof: THREE.Object3D<any>;
  body: THREE.Object3D<any>;
  window: THREE.Object3D<any>;
  base: THREE.Object3D<any>;
  mThruster: THREE.Mesh<THREE.CylinderGeometry, THREE.MeshPhongMaterial>;
  smokeArray: any[] = [];
  constructor() {
    ...
    let geoUpper = new THREE.CylinderGeometry(50, 75, 80, 16);
    let geoMiddle = new THREE.CylinderGeometry(75, 85, 80, 16);
    let geoColumn = new THREE.CylinderGeometry(85, 85, 200, 16);
    let geoWindowFrameOuter = new THREE.CylinderGeometry(55, 55, 40, 16);
    ...
    this.mThruster = new THREE.Mesh(geoThruster, matWindowFrame);
    this.mThruster.position.y = -305;
    this.mThruster.receiveShadow = true;
    this.mThruster.castShadow = true;
    ...
    this.base.add(this.mThruster, mConnector, mBurner);
    this.addSmoke();
  ...
  addSmoke() {
    for (let i = 0; i < 10; i++) {
      let smokeScale = 60 + Math.random() * 0.001;
      var nLines = 1 + Math.floor(Math.random() * 10);
      var nRows = 1 + Math.floor(Math.random() * 10);
      let smokeGeometry = new THREE.SphereGeometry(smokeScale, nLines, nRows);
      let smokeMaterial = new THREE.MeshLambertMaterial({
        color: this.getFlameColor(),
      });

      let smokeMesh = new THREE.Mesh(smokeGeometry, smokeMaterial);
      smokeMesh.position.y = -390;
      smokeMesh.userData.isUp = i % 2 === 0;
      smokeMesh.position.x = Math.random() * 30;
      smokeMesh.position.z = Math.random() * 30;
      this.base.add(smokeMesh);
      this.smokeArray.push(smokeMesh);
    }
    for (let i = 0; i < 16; i++) {
      let smokeScale = 3 + Math.random() * 10;
      var nLines = 3 + Math.floor(Math.random() * 2);
      var nRows = 3 + Math.floor(Math.random() * 2);
      let smokeGeometry = new THREE.SphereGeometry(smokeScale, nLines, nRows);
      let smokeMaterial = new THREE.MeshLambertMaterial({
        color: this.getFlameColor(),
      });
      ...
      let smokeMesh = new THREE.Mesh(smokeGeometry, smokeMaterial);
      smokeMesh.position.y = -950;
      smokeMesh.userData.isUp = i % 2 === 0;
      smokeMesh.userData.isSmall = true;
      this.base.add(smokeMesh);
      this.smokeArray.push(smokeMesh);
    }
  }

  animateSmoke(boost: boolean = false) {
    this.smokeArray.forEach((smokeParticle) => {
      smokeParticle.userData.isUp = !smokeParticle.userData.isUp;
      if (
        smokeParticle.userData.isSmall &&
        smokeParticle.position.y < -560 - Math.random() * 500
      ) {
        smokeParticle.position.y = -460;
        smokeParticle.position.x = Math.random() * 100;
        smokeParticle.position.z = Math.random() * 100;
      } else if (
        !smokeParticle.userData.isSmall &&
        smokeParticle.position.y < -450 - Math.random() * 100
      ) {
        // smokeParticle.material.color = this.getFlameColor(boost);
        smokeParticle.position.y = -420;
        smokeParticle.position.x = Math.random() * 50;
        smokeParticle.position.z = Math.random() * 50;
      }
      smokeParticle.position.x -=
        (smokeParticle.userData.isUp ? 1 : -1) * Math.random() * 2;
      smokeParticle.position.y -= Math.random() * 5;
      smokeParticle.position.z -=
        (smokeParticle.userData.isUp ? 1 : -1) * Math.random() * 2;
    });
  }

  getFlameColor(boost: boolean = false) {
  ...
  changeFlameColor() {
    this.smokeArray.forEach((smokeParticle) => {
      if (!smokeParticle.userData.isSmall) {
        gsap.to(smokeParticle.scale, {
          x: 1.8,
          y: 1.8,
          z: 1.8,
          duration: 1,
        ...
        if (Math.floor(Math.random() * 100) % 2 == 0) {
          smokeParticle.material.color.set(
            this.rgbToHex(
              Math.floor(220 + Math.random() * 35),
  ...
  removeSmoke() {
    this.smokeArray.forEach((smokeParticle) => {
      this.base.remove(smokeParticle);
    });
  }
